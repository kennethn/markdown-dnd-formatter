...
\begin{tcolorbox}[
  ...
  fontupper={\blockquoteFont\selectfont}
    ...
    for i, b in ipairs(el.content) do
      if i == 1 and b.t == 'Para' then
        local icon = pandoc.RawInline('latex', [[\footnotesize\color{imageborder}\faDungeon\hspace{0.8em}\selectfont\begin{minipage}[t]{\dimexpr\linewidth-1.8em\hangindent=1.8em\hangafter=0}]])
